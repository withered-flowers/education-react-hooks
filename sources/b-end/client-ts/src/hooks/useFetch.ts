...
import { ReqResInResponse } from "../ts-related/reqresin";

// di sini kita akan mencoba untuk membuat custom hooks
// Karena ini adalah sebuah "function"
// maka ini bisa menerima parameter
const useFetch = (urlToFetch: string) => {
  // Karena ini dianggap sebagai sebuah "component"
  // maka kita bisa menggunakan hooks di dalam sini
  const [isLoading, setIsLoading] = useState<boolean>(true);

  // Karena di sini menggunakan TS, maka data awal akan diset sebagai
  // undefined (maka nanti harus di cek isinya terlebih dahulu)
  const [data, setData] = useState<ReqResInResponse>();
